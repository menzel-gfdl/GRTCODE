...
/** @brief Read in ppmv values and interpolate from pressure layers to pressure levels.*/
...
/** @brief Read in "global mean" ppmv values.*/
                       ...
                       int const num_cfcs, int const * const cias, int const num_cias)
{
    ...
    for (i=0; i<atm->num_columns; ++i)
    {
        ...
        {
            atm->surface_emissivity[i*atm->num_wavenumber+j] = buffer[i];
        }
    }
    free(buffer);
...
void write_fluxes(Output_t const * const o, int const index, int const column,
                  fp_t const * const flux)
{
    size_t len;
