...
{
    IncompleteBeta_t beta;
    Device_t device;
    IceCloudOptics_t ice;
    OpticalProperties_t ice_optics;
    HuStamnes_t liquid;
    OpticalProperties_t liquid_optics;
    TotalWaterPDF_t water_pdf;
} Clouds_t;


/* @brief Read in and store data for the clouds.*/
int create_clouds(Clouds_t * const clouds, char const * beta_path,
                  char const * ice_path, char const * liquid_path,
                  int const * beta_shape, Device_t const * const device)
{
    not_null(clouds);
    not_null(beta_path);
    not_null(ice_path);
...
/* @brief Free memory for the clouds.*/
int destroy_clouds(Clouds_t * const clouds)
{
    destruct_beta(&beta);
