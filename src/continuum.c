              ...
              int maxwavenum)
{
    ...
    parseCKD(testCKDS_f,
             T0,
             maxwavenum);
    parseCKD(testMTCKDF_f,
             CF,
             maxwavenum);
    parseCKD(testMTCKDS_f,
             CS,
             maxwavenum);
}

void get_PD(REAL_t * const PD,
...
void get_PF(REAL_t * const PF,
            REAL_t const * const P,
            REAL_t const * const PD,
            const size_t nlyr)
{
    /*Local variables*/
    unsigned int n;
...
void get_TF_TD(REAL_t * const TF,
               REAL_t * const TD,
...
/*Calculate the optical depths associated with the continuum.*/
             ...
             const size_t NLAYERS)
{
    ...
    TBUF = (REAL_t *)calloc(2*NLINES*NLAYERS,
                            sizeof(REAL_t));
    if (TBUF == NULL)
    {
        fprintf(stderr,
