                      ...
                      char **vals)
{
    ...
    if (s <= 0 || s >= MAXCHARSPERTOKEN)
    {
        ...
        fatal("the size (%d) of the token %s on line %d must be in the"
                  " range [1,%d].",
              s,
              token,
              line_index+1,
              ...
              char *** out)
{
    ...
    if (f == NULL)
    {
        fatal("failed to open csv file %s.",
              filepath);
    }
    ...
    *num_cols = -1;
    while (fgets(line,MAXCHARSPERLINE,f) != NULL)
    {
        ...
        if (*c == '\n')
        {
            fatal("line %d in file %s is blank.",
                  *num_lines,
                  filepath);
        }
        int n = 1;
        ...
        while (*c != '\n')
        {
            ...
            {
                fatal("the number of characters (>=%d) on line %d of file %s"
        ...
        {
            fatal("the number of columns (%d) on line %d of file"
    ...
    *out = vals;
    ...
    if (0 != fclose(f))
    {
        fatal("failed to close csv file %s.",
              filepath);
    }
    return SUCCESS;
}
