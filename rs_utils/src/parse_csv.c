...
/** @brief Copy an input token into an input array.
    ...
    *num_cols = -1;
    while (fgets(line, MAXCHARSPERLINE, f) != NULL)
    {
        ...
        if (*c == '\n')
        {
            ...
            raise(RS_VALUE_ERR, mesg, *num_lines, filepath);
        }
        int n = 1;
        int num_chars = 0;
        while (*c != '\n')
        {
            ...
            if (num_chars > MAXCHARSPERLINE)
            {
                ...
                raise(RS_VALUE_ERR, mesg, num_chars, *num_lines, filepath, MAXCHARSPERLINE);
            }
        }
        if (*num_cols < 0)
        {
        ...
        else if (n != *num_cols)
        {
            mesg = "the number of columns (%d) on line %d of file"
                   ...
                   " the other lines.";
            raise(RS_VALUE_ERR, mesg, n, *num_lines, filepath, *num_cols);
        }
    }
    if (*num_lines == 0)
    {
        mesg = "the file %s is empty.";
        raise(RS_VALUE_ERR, mesg, filepath);
    }
