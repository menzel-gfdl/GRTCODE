...
/** @brief Perform scaling required by the delta_Eddington Method.  The scaling
           parameters are described in
           https://doi.org/10.1175/1520-0469(1976)033<2452:TDEAFR>2.0.CO;2.
    @return RS_SUCCESS or an error code.*/
HOST DEVICE static int delta_eddington_scaling_jww1976(fp_t const omega, /**< Single-scattering albedo.*/
                                                       fp_t const g, /**< Asymmetric factor.*/
                                                       fp_t const tau, /**< Optical depth.*/
                                                       fp_t * const omega_s, /**< Scaled single-scattering albedo.*/
                                                       fp_t * const g_s, /**< Scaled asymmetric factor.*/
...
/** @brief Calcluate the reflectivity and transmittance of a layer using the method
           described in
           https://doi.org/10.1175/1520-0469(1980)037<0630:TSATRT>2.0.CO;2.
    @return RS_SUCCESS or an error code.*/
HOST DEVICE static int meador_weaver_1980(fp_t const omega, /**< Scaled single-scattering albedo.*/
                                          fp_t const tau, /**< Optical depth.*/
...
/** @brief Use the Eddington approximation to calculate the reflectivity and
           ...
           https://doi.org/10.1175/1520-0469(1980)037<0630:TSATRT>2.0.CO;2.
    @return RS_SUCCESS or an error code.*/
HOST DEVICE static int eddington_mw1980(fp_t const omega, /**< Scaled single-scatterting albedo.*/
                                        fp_t const tau, /**< Scaled layer optical depth.*/
                                        fp_t const mu, /**< Cosine of the solar zenith angle.*/
                                        fp_t const g, /**< Scaled asymmetric factor.*/
                                        fp_t * const R, /**< Layer reflectivity.*/
                                        fp_t * const T, /**< Layer transmittance.*/
                                       ...
                                       )
{
    /*Calculate the Eddington approximation parameters, as defined in
      row 1 of table 1 in Meador & Weaver (1980).*/
    clear_floating_point_exceptions();
...
/** @brief Calculate the shortwave upward and downward fluxes using the "adding"
...
HOST DEVICE static int sw_adding(int const nlevels, /**< Number of atmospheric pressure levels.*/
                                 fp_t const * const R_dir, /**< Reflectivity of each atmospheric layer for a
                                                                direct beam.*/
                                 fp_t const * const R_dif, /**< Reflectivity of each atmospheric layer for a
                                                                diffuse beam.*/
                                 fp_t const * const T_dir, /**< Transmittance of each atmospheric layer for a
                                                                direct beam.*/
                                 fp_t const * const T_dif, /**< Transmittance of each atmospheric layer for a
                                                                diffuse beam.*/
                                 ...
                                 fp_t * const R, /**< Total upward reflectance at each atmospheric
                                                      pressure level.*/
                                 fp_t * const T /**< Total downward reflectance at each atmospheric
                                                     pressure level.*/
                                )
{
    not_null(R_dir);
    ...
    fp_t R_dir_downward[MAX_NUM_LEVELS]; /*Reflectance of each
                                           atmospheric layer
                                           for a downward traveling
                                           direct beam.*/
    fp_t R_dif_downward[MAX_NUM_LEVELS]; /*Reflecance of each
                                           atmospheric layer
                                           for a downward traveling
                                           diffuse beam.*/
    fp_t R_dif_upward[MAX_NUM_LEVELS]; /*Reflectance of each
                                         atmospheric layer for
                                         an upward traveling
                                         diffuse beam.*/
...
/** @brief Calculate the upward and downward shortwave radiative fluxes
...
HOST DEVICE static int sw_flux(int const nlevels, /**< Number of atmospheric pressure levels.*/
                               fp_t const * const omega, /**< Single-scattering albedo of each
                                                              atmospheric layer.*/
                               fp_t const * const g, /**< Asymmetric factor of each atmospheric
                                                          layer.*/
                               fp_t const * const tau, /**< Optical depth of each atmospheric
                                                            layer.*/
                               fp_t const mu_dir, /**< Cosine of the zenith angle for a
                                                       direct beam.*/
                               fp_t const mu_dif, /**< Cosine of the zenith angle for a diffuse
                                                       beam.*/
                               fp_t const sfc_alpha_dir, /**< Albedo of the Earth's surface for
                                                              a direct beam.*/
                              ...
                              )
{
    ...
    /*Local buffers.*/
    int const nlayers = nlevels - 1;
    fp_t R_dir[MAX_NUM_LEVELS-1]; /*Reflectivity of each atmospheric
                                    layer for a direct beam.*/
    fp_t R_dif[MAX_NUM_LEVELS-1]; /*Reflectivity of each atmospheric
                                    layer for a diffuse beam.*/
    fp_t T_dir[MAX_NUM_LEVELS-1]; /*Transmittance of each atmospheric
                                    layer for a direct beam.*/
    fp_t T_dif[MAX_NUM_LEVELS-1]; /*Transmittance of each atmospheric
                                    layer for a diffuse beam.*/
    fp_t T[MAX_NUM_LEVELS-1]; /*Amount of the incident beam that makes it through
                                each layer without being absorbed or scattered.*/
...
/** @brief Calculate the shortwave fluxes in a column at each wavenumber.
    @return RS_SUCCESS or an error code.*/
...
#pragma omp parallel for schedule(static) default(none) private(i)
    for (i=0; i<num_wpoints; ++i)
    {
        ...
        sw_flux(num_levels, omega_buf, g_buf, tau_buf, mu_dir, mu_dif, sfc_alpha_dir,
                sfc_alpha_dif, solar_flux[i], flux_up_buf, flux_down_buf);
                                         ...
                                         )
{
    ...
    if (tid < num_wpoints)
    {
        ...
        sw_flux(num_levels, omega_buf, g_buf, tau_buf, mu_dir, mu_dif, sfc_alpha_dir,
                sfc_alpha_dif, solar_flux[tid], flux_up_buf, flux_down_buf);
        for (j=0; j<num_levels; ++j)
        {
            uint64_t offset = j*num_wpoints + tid;
            flux_up[offset] = total_solar_irradiance*flux_up_buf[j];
...
EXTERN int sw_get_spectral_grid_size(Shortwave_t const * const sw, uint64_t * const n)
{
    not_null(sw);
    not_null(n);
    *n = sw->grid.n;
